...
import { searchMovies, searchTvShows, formatLanguage } from "@/lib/tmdb"
import { mediaDetailPath } from "@/lib/media"
import type { MediaItem, MediaType } from "@/lib/types"

type SearchTab = MediaType
...
export default function MediaSearch({
  ...
  onAdd: (item: MediaItem) => Promise<{ ok: boolean; message?: string }>
  onClose: () => void
}) {
  const router = useRouter()
  const [tab, setTab] = useState<SearchTab>("movie")
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(async () => {
      ...
      const items =
        tab === "tv"
          ? await searchTvShows(query, parsedYear)
          : await searchMovies(query, parsedYear)
      setResults(items)
      setLoading(false)
    }, 400)
  ...
  const placeholder = tab === "tv" ? "Search for a TV show..." : "Search for a movie..."
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "color-mix(in srgb, var(--background-base) 80%, transparent)" }}
      />
      ...
      >
        <div className="mb-2 flex gap-2">
          {(["movie", "tv"] as const).map((type) => (
            <button
              ...
              type="button"
              onClick={() => setTab(type)}
              className="t-button-sm rounded-full px-4 py-2 transition-colors"
              style={tab === type ? activePillStyle : inactivePillStyle}
            >
        ...
        {(queryReady && (results.length > 0 || loading)) && (
          <div
            className="mt-2 max-h-[60vh] overflow-y-auto rounded-xl border"
            style={{ background: "var(--background-mid)", borderColor: "var(--border-hairline)" }}
          >
            {loading && results.length === 0 && (
              <div className="t-meta px-5 py-8 text-center text-white/30">Searching...</div>
            )}
            ...
            {results.map((item) => (
              ...
              >
                <button
                  ...
                  onClick={() => {
                    onClose()
                    router.push(mediaDetailPath(item))
                  }}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="t-title-sm truncate text-white/90">{item.title}</p>
                    <p className="t-caption text-white/40">
                      {formatLanguage(item.language)} · {item.year}
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      onClose()
                      router.push(`${mediaDetailPath(item)}?rate=1`)
                    }}
